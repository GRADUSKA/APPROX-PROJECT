...
# Listes de noms possibles (pour simplification, nous pouvons générer des noms comme Champ1, Champ2, etc.)
...
for i in range(num_champions):
    ...
    # Générer entre 1 et 3 traits pour chaque champion
    num_traits = random.randint(2, 3)
    
    traits = set()
    while len(traits) < num_traits:
        if len(traits) < 2:  # Ensure a mix of origins and classes
...
# Formater les résultats selon la grammaire spécifiée
